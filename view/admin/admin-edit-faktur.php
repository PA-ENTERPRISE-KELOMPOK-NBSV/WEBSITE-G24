...
if (isset($_POST['kurangi_produk'])) {
    ...
    // Kurangi jumlah produk dalam detail pemesanan
    ...
    if (mysqli_query($conn, $query_kurangi_jumlah)) {
        // Perbarui total barang dan total harga di pemesanan
        if (updatePemesananTotals($conn, $no_pemesanan)) {
            header('location: admin-edit-faktur.php?no_pemesanan=' . $no_pemesanan);
            exit;
        } else {
    ...
    if ($result_produk && mysqli_num_rows($result_produk) > 0) {
        ...
        if ($count_produk > 0) {
            ...
            if (mysqli_query($conn, $query_update_jumlah)) {
                // Jika berhasil memperbarui jumlah, perbarui total barang dan total harga
                if (updatePemesananTotals($conn, $no_pemesanan)) {
                    header('location: admin-edit-faktur.php?no_pemesanan=' . $no_pemesanan);
                    exit;
                } else {
            ...
            // Jika produk belum ada, tambahkan ke detail pemesanan
            ...
            if (mysqli_query($conn, $query_insert_detail)) {
                // Jika berhasil menambahkan, perbarui total barang dan total harga
                if (updatePemesananTotals($conn, $no_pemesanan)) {
                    header('location: admin-edit-faktur.php?no_pemesanan=' . $no_pemesanan);
                    exit;
                } else {
    ...
    while ($row = mysqli_fetch_assoc($result_produk)) {
        ...
        <tr>
            ...
            <form method="post">
                ...
                <?php else: ?>
                    <?php if ($row['jumlah'] > 1): ?>
                        <button type="submit" class="btn btn-secondary btn-minus" name="kurangi_produk" disabled>-</button>
                        <!-- <button type="submit" class="btn btn-primary btn-plus">+</button> -->
                        <?php else: ?>
                            <button type="button" class="btn btn-secondary btn-minus" disabled>-</button>
                    <?php endif; ?>
                                                ...
                                                <div class="col-xl-5 invoice-address-client">
                                                    ...
                                                    <div class="invoice-address-client-fields">
                                                        ...
                                                        if ($result_order->num_rows > 0) {
                                                            ...
                                                            // Isi input nama dengan data dari URL (GET), jika tidak tersedia, gunakan "-"
                                                            $nama = !empty($order_data['nama']) ? $order_data['nama'] : '';
                                                            // Isi input alamat dengan data dari URL (GET), jika tidak tersedia, gunakan "-"
                                                            ...
                                                            <tbody>
                                                                ...
                                                                if ($result_detail->num_rows > 0) {
                                                                    ...
                                                                    while ($detail_data = $result_detail->fetch_assoc()) {
                                                                        ?>
                                                                        <tr>
                                                                            <td class='delete-item-row' onclick="hapusProduk('<?php echo $detail_data['produk_id']; ?>')">
                                                                    ...
                                                                    // Tampilkan pesan jika detail pemesanan tidak ditemukan
...
<script>
    // Menonaktifkan dropdown saat halaman dimuat
    document.addEventListener("DOMContentLoaded", function() {
        ...
        dropdownToggle.addEventListener("click", function(event) {
            event.preventDefault(); // Mencegah tindakan default dropdown saat diklik
        });
    });
...
<script>
    ...
    $.ajax({
        url: 'admin-edit-status.php', // Sesuaikan dengan file PHP yang akan menangani pembaruan status
        type: 'POST',
        dataType: 'json',
        data: {
            update_status: true,
        ...
        success: function(response) {
            if (response.status === 'success') {
                // Status berhasil diperbarui, lakukan tindakan tambahan jika diperlukan
                alert('Status pemesanan berhasil diperbarui');
                // Lakukan pengalihan atau tindakan lain yang diperlukan setelah pembaruan berhasil
                window.location.reload(); // Contoh: Muat ulang halaman
            } else {
                // Gagal memperbarui status, tampilkan pesan kesalahan
